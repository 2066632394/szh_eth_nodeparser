...
if (cluster.isMaster) {
	...
	var _33ContractAddress = "0x2e347ef1b9b036ceb3d2733afb63a3c3fc5cb79e"; 
	...
	var _33CoindbContract = web3.eth.contract(abiJson);
	var _33Coindb = _33CoindbContract.at(_33ContractAddress);
	...
	function parseBlock(block) {
		...
		for (var j = 0; j < txs.length; j++) {
			if(txs[j].to != _33ContractAddress) {
				continue;	
			} else {
				...
				for (var k = 0; k < receipts[j].logs.length; k++) {									
					_33Coindb.decodeLog(receipts[j].logs[k]);
				}
	...
	function parseTxInput(input) {
		var match = abiJson.filter(function (jsondata) {			
			...
			if (!_33Coindb[jsondata.name] || !_33Coindb[jsondata.name].signature) {
				return false;
			}
			if (_33Coindb[jsondata.name].signature() == input.slice(2, 10)) {
				return true;
			}
		})
		var inputDecode = [];		
		...
		inputDecode["args"] = _33Coindb[match[0].name].unpackArgs(input);
	...
	function parseContract(tx, receipt) {
		//console.log(tx, "here");
		/*if (typeof receipt["logs"] == "undefined" || receipt["logs"].length == 0){
			return null;
		}*/
	...
	function insertPlaceOrders(tx, receipt) {
		...
		DB.insert("33_placeorders", insertData);
	}
	// 插入cancelorder
	function insertCancelOrders(tx, receipt) {
		...
		//console.log(insertData);
		DB.insert("33_cancelorders", insertData);
	}
	// 插入matchOrder
	function insertMatchOrders(tx, receipt) {
		...
		//console.log(insertData);
		DB.insert("33_matchorders", insertData);
	}
	// 插入sendcoin
	function insertSendCoins(tx, receipt) {
		...
		//console.log(insertData);
		DB.insert("33_sendcoins", insertData);
	}	
}	
