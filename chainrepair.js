...
if (cluster.isMaster) {
    ...
    function getMaxBlock(){
		...
		DB.findOne("blocks", "*", callMaxCount, {'limit':1, 'order':order});
    }

	// 获取数据库不连续的block区块 修复添加到mysql中
	function getLostBlock() {

		DB.querySql("select * from blocks", callGetLoopLostBlock,{'limit':10});
	}
	// 获取数据库最大的blocknumber回调
	function callGetLoopLostBlock(rows) {
		console.log(rows);
		console.log('repair block length----'+rows.length);
		if(rows.length != 0) {
			rows.forEach(function(row){
				var stime = new Date().getTime();
				console.log('repair block -----'+row.block);
				if(row.length !=0  && row.block != 'undefined'){
					web3.eth.getBlock(row.block,true,function(err,block){
						if(err != null){
							console.log(err)
			...
			console.log('no block need to repair');
		}
	}	
		...
		for (var j = 0; j < txs.length; j++) {
			...
			if(getContractAddersses().indexOf(txs[j].to) == "-1") {	
				...
				if(contractConfig[txs[j].to].script != "" 
					...
					var insertContract = require('./'+contractConfig[txs[j].to].script);
					txs[j]['timestamp'] = block.timestamp;
