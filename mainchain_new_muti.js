...
if (cluster.isMaster) {
	...
	var DB = new Dbobject(dbConfig.pooldb2);
	DB.createPool();
	...
	function callGetLoopStartBlock(row) {
		...
		if(top != 'undefined'){
			if(top>=loopStart){
				//top = 998;
				var group = top-loopStart;
				if(group>1000){
					top = loopStart + 1000-1;
				}
				for (var i = loopStart; i <top+1 ; i++) {
					list.push(i);
				}
				
				async.eachLimit(list,2,function(item,callback){
	...
	// 		//return;
	// 	} else {
	// 		client.hgetallAsync(['block',loopStart]).then(function(bres){
	// 			if(bres){
	// 				console.log('redis block :'+bres);
