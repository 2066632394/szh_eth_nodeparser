                ...
                if(receipts[j] != null){
                    ...
                    for (var k = 0; k < receipts[j].logs.length; k++) {	
                        ...
                        if(receipts[j].logs[k]['event'] == "UpdateWallet"
                                ...
                                || receipts[j].logs[k]['event'] == "SetFeeRate"
                                        ...
                                        if (tErr) {
                                            ...
                                            connection.query("update bill_accounts set active = active+? where addr = ? and currency = ?", [rmb,borrower,1], function (tErr, rows, fields) {
                                                if (tErr) {
                                                    ...
                                                    connection.query("update bill_list set bill = bill-?,unsellcount = unsellcount-?,rmb = rmb + ?  where bill_id = ?", [pay,num,rmb,billId], function (tErr, rows, fields) {
                                                        if (tErr) {
                                                            connection.rollback(function () {
                                                                console.log("事务失败，，ERROR：" + tErr);
                                                                throw tErr;
            ...
            var temp = function(cb){
                connection.query(sql['sql'], sql_params, function (tErr, rows, fields) {
                    if (tErr) {
                        ...
                        connection.query(sql['sql'], ['fee2',fee2,fee2], function (tErr, rows, fields) {
                            if (tErr) {
                                connection.rollback(function () {
                                    console.log("事务失败，，ERROR：" + tErr);
                                    throw tErr;
