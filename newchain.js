...
if (cluster.isMaster) {
	...
	function blockLoop(){
		...
		if(loopStart > topblock) {
			...
			var arr = [];
			for (var i = loopStart; i < (topblock+1) ; i++) {
				arr.push(i);
			}
			//console.log(arr);
			...
			async.eachSeries(arr,function(item,callback){
					...
					web3.eth.getBlock(item,true,function(err,block){
						if(err)	{
							...
							//setTimeout(getLoopStartBlock, 1000);
						}
						var etim = new Date().getTime();
						//console.log(item+"---------get block time ----------------"+(etim-stime));
						//console.log('block : '+block.number+' size : '+block.size);
						parseBlock(block, function(berr,bresult){
							...
							console.log(item+"--------parse block time -----------------"+(etime-etim)+"---------get block time ----------------"+(etim-stime));
						});
	...
	function parseBlock(block,bcallback) {
		...
		for (var j = 0; j < txs.length; j++) {			
			if(getContractAddersses().indexOf(txs[j].to) == "-1") {	
				//logger.error(" no contract  address in config like "+txs[j].to)
			} else {
		                        	...
		                        	connection.query("update trade_market set amount = amount-? where price = ? and ty = 1",[amount,bprice] , function (tErr5, rows5, fields5) {
		                        		if (tErr5) {
				                        	...
				                        	connection.query("update trade_order set unmatch = unmatch -? ,update_time = ? ,cost = cost+ ? where id in (?,?)", [amount,log['args']['timestamp'],cost,offerid,wantedid], function (tErr1, rows1, fields1) {
								                	...
								                	connection.query("update trade_matches set cost_price = ?,type= ?,symbol = ? where id = ?", [sprice,active_type,active_symbol,mtfId], function (tErr4, rows4, fields4) {
										                    if (tErr4) {
										                        connection.rollback(function () {
...
// sql['params'] = [];
// var sql2 = [];
// sql2['sql'] = 'insert into user (name,content) values (?,?)';
// sql2['params'] = ["heloo","test"];

// sql_arr.push(sql);
// sql_arr.push(sql2);
// DB.execTrans(sql_arr,function(err,result){
// 	if(err){
// 		console.log('fail');
// 	}else{
// 		console.log('succ');
...
// });
  ...
  //     	var sql = "insert into user (name) values ('"+i+"') ";
...
// connection.beginTransaction(function(err) {
//   if (err) { throw err; }
//   connection.query('INSERT INTO user SET name=?', title, function(err, result) {
//     if (err) {
