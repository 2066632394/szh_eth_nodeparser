...
if (cluster.isMaster) {
	...
	function blockLoop(){
		...
		var group = topblock-loopStart;
		var limit = 5;
		var arr = [];
		if(group >1000){
			group = 1000;
		}
		for (var i = loopStart+1; i < (loopStart+group+1) ; i++) {
			arr.push(i);
		}
		// for (var i = 1138; i < 1660 ; i++) {
		// 	arr.push(i);
		// }
		//console.log(arr);
		async.eachLimit(arr,limit,function(item,callback){
				...
				setTimeout(getLoopStartBlock1,500);
			}
