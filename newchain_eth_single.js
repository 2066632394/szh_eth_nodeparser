...
if (cluster.isMaster) {
    ...
    var contractConfig = require('./config/contract_eth_single.js');
    // 获取配置合约地址集合	
    ...
    function blockLoop(){
        ...
        if(loopStart > topblock) {
            ...
            arr = [104366,104368,104369,104370,104372,104389,104426,104440,104441,104445],
    ...
    function parseLogs(log,connection){
        ...
        if(log['event'] == "UpdateWallet"){
            ...
            var sql = "INSERT INTO evi_list (address,name,hash,addtime) values (?,?,?,?) ";

            var temp = function(cb){
                connection.query(sql, [fr,name,hash,log['args']['timestamp']], function (tErr, rows, fields) {
                    if (tErr) {
                        connection.rollback(function () {
                            console.log("事务失败，，ERROR：" + tErr);
                            throw tErr;
